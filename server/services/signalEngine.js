...
function init(socketIO) {
  ...
  === TARGET ASSETS ===
  Generate signals for these tickers that are most exposed to the trigger event(s):
  ${tickerList.join(', ')}
  
  === YOUR TASK ===
  Cross-correlate the TRIGGER EVENT with each asset's specific business exposure. Be SPECIFIC about why each event affects each ticker.
    ...
    "signals": [
      {
        ...
        "confidence": <0-100>,
        "reasoning": "2-3 sentence specific analysis connecting the trigger event to this ticker",
        "geopoliticalFactors": ["2-3 specific drivers"],
        "riskFactors": ["2 factors that could disrupt this assessment"],
        "timeHorizon": "<SHORT|MEDIUM|LONG>",
        "correlatedAssets": ["3 related tickers"],
        "stopLossReasoning": "Specific trigger to invalidate",
    ...
    "watchlist": [
      {
        ...
        "reasoning": "1-sentence why this event affects this asset",
        "impactDirection": "<POSITIVE|NEGATIVE>"
      }
    ]
  }`;
  
      console.log('[SignalEngine] 🧠 Calling AI for auto-signal generation...');
      const aiResult = await generateAI(prompt);
  
      if (!aiResult || !Array.isArray(aiResult.signals)) {
        console.warn('[SignalEngine] AI returned no signals, generating fallback');
        emitFallbackSignals(criticalEvents, tickerList, triggerEventTitle);
        return;
      }
  
      // Process and save signals
      const processedSignals = [];
      for (const raw of aiResult.signals) {
        const signal = {
          ticker: raw.ticker || tickerList[0],
          signal: ['BUY', 'HOLD', 'SELL'].includes(raw.signal) ? raw.signal : 'HOLD',
          confidence: Math.min(100, Math.max(0, parseInt(raw.confidence) || 50)),
          reasoning: raw.reasoning || 'Auto-generated signal based on critical event analysis.',
          geopoliticalFactors: Array.isArray(raw.geopoliticalFactors) ? raw.geopoliticalFactors.slice(0, 3) : [],
          riskFactors: Array.isArray(raw.riskFactors) ? raw.riskFactors.slice(0, 2) : [],
          timeHorizon: ['SHORT', 'MEDIUM', 'LONG'].includes(raw.timeHorizon) ? raw.timeHorizon : 'SHORT',
          correlatedAssets: Array.isArray(raw.correlatedAssets) ? raw.correlatedAssets.slice(0, 3) : [],
          stopLossReasoning: raw.stopLossReasoning || '',
          triggerType: 'auto',
          triggerEvent: triggerEventTitle,
          createdAt: new Date(),
        };
        ...
        if (SignalHistory) {
          ...
          } catch (e) { console.warn('[SignalEngine] DB save error:', e.message); }
        }
      }
  
      // Process watchlist
      const watchlist = Array.isArray(aiResult.watchlist)
        ? aiResult.watchlist.slice(0, 5).map(w => ({
            symbol: w.symbol || '',
            name: w.name || w.symbol || '',
            reasoning: w.reasoning || '',
            impactDirection: ['POSITIVE', 'NEGATIVE'].includes(w.impactDirection) ? w.impactDirection : 'NEGATIVE',
          }))
        : tickerList.map(t => ({
            symbol: t,
            name: t,
            reasoning: `Directly exposed to: ${triggerEventTitle}`,
      ...
      if (io) {
        const payload = {
          ...
          triggerEvent: {
            title: triggerEventTitle,
            country: criticalEvents[0]?.country || 'GLOBAL',
            severity: 'CRITICAL',
          },
          timestamp: new Date().toISOString(),
        };
  
        io.emit('signal:autoGenerated', payload);
        io.emit('signal:watchlist', {
          tickers: watchlist,
          triggerEvent: triggerEventTitle,
          timestamp: new Date().toISOString(),
        });
  
        console.log(`[SignalEngine] ✅ Pushed ${processedSignals.length} auto-signals + ${watchlist.length} watchlist items to all clients`);
      }
  
    } catch (err) {
      console.error('[SignalEngine] Auto-signal generation error:', err.message);
    }
  }
  /**
   * Emit fallback signals when AI is unavailable
   */
  function emitFallbackSignals(criticalEvents, tickerList, triggerEventTitle) {
    const fallbackSignals = tickerList.map(ticker => ({
      ticker,
      signal: 'HOLD',
      confidence: 55,
      reasoning: `Auto-generated: CRITICAL event detected — "${triggerEventTitle}". Elevated uncertainty warrants caution. Monitor for further developments before taking positions.`,
      geopoliticalFactors: [triggerEventTitle],
      riskFactors: ['Rapid de-escalation could invalidate this assessment', 'Market may have already priced in the event'],
      timeHorizon: 'SHORT',
      correlatedAssets: tickerList.filter(t => t !== ticker).slice(0, 3),
      stopLossReasoning: 'Exit if the triggering event is resolved diplomatically within 48 hours.',
      triggerType: 'auto',
      triggerEvent: triggerEventTitle,
      createdAt: new Date(),
    }));
  
    const fallbackWatchlist = tickerList.map(t => ({
      symbol: t,
      name: t,
      reasoning: `Potentially exposed to: ${triggerEventTitle}`,
      impactDirection: 'NEGATIVE',
    }));
  
    if (io) {
      io.emit('signal:autoGenerated', {
        signals: fallbackSignals,
        triggerEvent: { title: triggerEventTitle, country: criticalEvents[0]?.country || 'GLOBAL', severity: 'CRITICAL' },
        timestamp: new Date().toISOString(),
      });
      io.emit('signal:watchlist', {
    ...
    // Save fallbacks to DB too
    if (SignalHistory) {
      fallbackSignals.forEach(async (s) => {
        try { await new SignalHistory(s).save(); } catch(e) {}
      });
    }
  
    console.log(`[SignalEngine] ⚠️ Pushed ${fallbackSignals.length} FALLBACK signals (AI unavailable)`);
  }
