...
function getSource(item) {
  ...
  if (src.includes("aljazeera") || src.includes("al-jazeera"))
    return "Al Jazeera";
...
const IntelligenceCard = ({ item, isTop, onSimulate }) => {
  ...
  const escalation =
    ESCALATION_COLORS[item.escalationRisk] || ESCALATION_COLORS.MEDIUM;
    ...
    >
      {isTop && (
        <div className="px-3 py-1 bg-black/40 border-b border-white/5 flex items-center gap-2">
          ...
          <span className="text-[var(--color-gold)] text-[9px] font-heading uppercase tracking-widest">
            Top Intel 24h
          </span>
        </div>
      )}
      ...
      <div className="p-3">
        ...
        {/* Summary */}
        <p
          className={`text-white/60 text-[11px] leading-relaxed mb-2 ${expanded ? "" : "line-clamp-2"}`}
        >
          {item.intelSummary ||
            "Deep neural analysis evaluating secondary impacts and cross-border escalation risks."}
        </p>

        {/* Escalation Risk + Source */}
        <div className="flex items-center gap-2 mb-2 flex-wrap">
          <span className="text-[8px] font-bold text-white/30 px-1.5 py-0.5 bg-white/5 border border-white/10 rounded-sm">
            {source}
          </span>
          ...
          {item.escalationRisk && (
            <span
              className="text-[7px] font-bold px-1.5 py-0.5 rounded-sm flex items-center gap-1"
              style={{
                background: escalation.bg,
                border: `1px solid ${escalation.border}`,
                color: escalation.text,
              }}
            >
              <i
        ...
        {item.impactSectors?.length > 0 && (
          <div className="flex items-center gap-1 mb-2 flex-wrap">
            {item.impactSectors.map((sector, i) => (
              <span
                key={i}
                className="px-1.5 py-0.5 text-[7px] font-bold uppercase tracking-wider bg-[var(--color-cyan)]/10 border border-[var(--color-cyan)]/20 rounded-sm text-[var(--color-cyan)]"
              >
                {sector}
              </span>
            ))}
            ...
            {item.relatedEvents && item.relatedEvents !== "Isolated event" && (
              <div className="bg-[var(--color-purple)]/5 border border-[var(--color-purple)]/15 rounded-sm px-2.5 py-2">
                <div className="text-[7px] font-bold text-[var(--color-purple)] uppercase tracking-[0.2em] mb-1 flex items-center gap-1">
                  <i className="fa-solid fa-link" style={{ fontSize: 7 }}></i>{" "}
        ...
        <div className="flex items-center justify-between pt-2 border-t border-white/5">
          ...
          <button
            ...
            className="bg-transparent border-none cursor-pointer pointer-events-auto"
          >
            <i
              className={`fa-solid fa-chevron-${expanded ? "up" : "down"} text-white/20 text-[9px] ml-2 hover:text-white/50 transition-colors`}
            ></i>
          </button>
        </div>
      </div>
...
export default function NewsPanel({
  ...
  return (
    <div
      className="flex flex-col h-full bg-[#0A0F1E]"
      style={{ overflow: "hidden" }}
    >
      <div
        className={`px-4 py-3 border-b border-white/10 flex items-center ${hideTitle ? "justify-end" : "justify-between"} gap-2`}
      >
        {!hideTitle && (
          <div className="flex items-center gap-2">
            <i className="fa-solid fa-book-bookmark text-[var(--color-red)] text-sm"></i>
            <h2 className="text-white font-heading uppercase tracking-[0.3em] text-[11px]">
              Smart Digest
            </h2>
      ...
      <div className="px-3 pb-3 flex items-center gap-1.5 flex-wrap">
        {["HIGH", "MEDIUM", "24H", "ESCALATION", "DE-ESCALATION"].map(
          (chip) => {
            const chipCount =
              chip === "24H"
                ? news.length
            ...
            return (
              ...
              >
                {chip === "ESCALATION" && (
                  <i className="fa-solid fa-arrow-trend-up mr-1 opacity-50"></i>
                )}
                {chip === "DE-ESCALATION" && (
                  <i className="fa-solid fa-arrow-trend-down mr-1 opacity-50"></i>
                )}
                {chip}
        ...
        {loading ? (
          ...
          filtered.map((item, i) => (
            <IntelligenceCard
              key={i}
              item={item}
              isTop={i === 0 && !search && activeChip === "HIGH"}
